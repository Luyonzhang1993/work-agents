...
choose one workflow from a small workflow catalog, such as
`workflow:finance_company_report`, or return no workflow. The router does not
...
`get_company_news`, and `get_financial_data` tools. MCP tools remain available
through `/api/mcp/*`, and workflows can use them internally when needed.
...
implemented by LLM calls instead of the mock finance MCP server; the final report
is also generated by the LLM from prior node outputs rather than a local template.
